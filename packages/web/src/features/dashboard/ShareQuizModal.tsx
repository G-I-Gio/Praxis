...
const ShareQuizModal = ({
  ...
  const [sharedWith, setSharedWith] = useState<string[]>(quiz.shared_with ?? [])
  ...
  const apply = async (nextShared: string[]) => {
    ...
    try {
      ...
      await onSetVisibility(quiz.id, visibility, nextShared)
      ...
      <div className="bg-background flex max-h-[80svh] w-full max-w-sm flex-col rounded-xl p-5 shadow-xl">
        ...
        <div className="mb-4 flex shrink-0 items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-muted-foreground truncate text-xs">{quiz.subject}</p>
          </div>
          <button
        ...
        <div className="mb-4 shrink-0">
          ...
          {hasAccess.length === 0 ? (
            <p className="text-muted-foreground text-xs italic">
              {t("manager:share.nobody", { subject: quiz.subject })}
            </p>
          ) : (
