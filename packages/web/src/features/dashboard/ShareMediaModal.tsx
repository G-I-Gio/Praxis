...
import type { MediaEntry } from "./useMediaApi"
...
interface Props {
  ...
  onClose:           () => void
  onSetVisibility:   (_id: string, _v: "private" | "public" | "shared", _sw: string[]) => Promise<void>
}

const ShareMediaModal = ({ media, allManagers, currentManagerId, onClose, onSetVisibility }: Props) => {
  ...
  const hasAccess = allManagers.filter((m) => sharedWith.includes(m.id) && m.id !== currentManagerId)
  ...
  const apply = async (nextShared: string[]) => {
    ...
    try {
      ...
      await onSetVisibility(media.id, visibility, nextShared)
      setSharedWith(nextShared)
      toast.success(nextShared.length > 0
      ...
      <div className="bg-background flex max-h-[80svh] w-full max-w-sm flex-col rounded-xl p-5 shadow-xl">
        <div className="mb-4 flex shrink-0 items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-muted-foreground truncate text-xs">{media.original_name}</p>
          </div>
          <button className="text-muted-foreground hover:text-foreground shrink-0 rounded-sm p-1" onClick={onClose}>
            <X className="size-4" />
          </button>
        </div>
        ...
        <div className="mb-4 shrink-0">
          ...
          {hasAccess.length === 0 ? (
            <p className="text-muted-foreground text-xs italic">
              {t("manager:share.nobody", { subject: media.original_name })}
            </p>
          ) : (
            <ul className="max-h-32 space-y-1.5 overflow-y-auto">
              {hasAccess.map((m) => (
                <li key={m.id} className="border-accent flex h-9 items-center justify-between rounded-md border-2 px-3">
                  ...
                  <button
                    ...
                    onClick={() => void apply(sharedWith.filter((x) => x !== m.id))}
                    disabled={busy}
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="relative mb-2 shrink-0">
            ...
            <Input variant="sm" value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder={t("manager:share.searchPlaceholder")} className="w-full pl-7" />
          </div>
          <ul className="min-h-0 flex-1 space-y-1.5 overflow-y-auto">
            {canAdd.length === 0 ? (
              <p className="text-muted-foreground py-2 text-center text-xs italic">
                {search ? t("manager:share.noResult", { search }) : t("manager:share.allHaveAccess")}
              </p>
            ) : canAdd.map((m) => (
              <li key={m.id} className="border-accent flex h-9 items-center justify-between rounded-md border-2 px-3">
                <span className="text-foreground text-sm font-medium">{m.username}</span>
                <button
                  ...
                  onClick={() => void apply([...sharedWith, m.id])}
                  disabled={busy}
        ...
        <Button className="bg-accent text-accent-foreground mt-4 shrink-0 w-full" onClick={onClose}>
